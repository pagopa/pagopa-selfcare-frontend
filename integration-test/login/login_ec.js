...
const login = (async () => {
    ...
    }
    {
        ...
        if (inputType === 'select-one') {
            await changeSelectElement(element, 's.freud')
        } else if ([
            'textarea',
            'text',
            'url',
            ...
            await typeIntoElement(element, 's.freud');
        } else {
            await changeElementValue(element, 's.freud');
        }
    }
    {
        const targetPage = page;
    ...
    }
    {
        ...
        const inputType = await element.evaluate(el => el.type);
        if (inputType === 'select-one') {
            await changeSelectElement(element, 'comune di comabbio')
        } else if ([
            'textarea',
            'text',
            'url',
            ...
            await typeIntoElement(element, 'comune di comabbio');
        } else {
            await changeElementValue(element, 'comune di comabbio');
        }
    }
    {
        const targetPage = page;
        await scrollIntoViewIfNeeded([
            [
                'aria/Comune di Comabbio'
            ],
            [
                'h6'
            ],
            [
                'xpath///*[@data-testid="PartyItemContainer: Comune di Comabbio"]/div/div/div[2]/h6'
            ],
            [
                'pierce/h6'
            ],
            [
                'text/Comune di Comabbio'
            ]
        ], targetPage, timeout);
        const element = await waitForSelectors([
            [
    ...
    console.log(cookies);
    saveDataToFile('cookies', cookies);


    // Dump local storage
    const localStorage = await page.evaluate(() => Object.assign({}, window.localStorage));
    console.log(localStorage);
    saveDataToFile('localStorage', localStorage);
    ...
    function saveDataToFile(name, data) {
        ...
        fs.writeFileSync(`./${name}.json`, dataToSave);
        console.log(`Saved ${name} to file: ./${name}.json`);
    }
