...
test.describe.serial('Public bundles flow', () => {
  ...
  const handleEcBundleActions = async (page: Page, action: 'activate' | 'deactivate', attempt?: string) => {
    ...
    if (action === 'activate') {
      const activated = await test.step(`Activate the public bundle${attempt ? ' ' + attempt : ''}`, async () => 
