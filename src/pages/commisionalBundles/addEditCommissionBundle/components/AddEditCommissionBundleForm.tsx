...
const AddEditCommissionBundleForm = ({ isEdit, formik, idBrokerPsp }: Props) => {
  ...
  const [filterChannelCode, setFilterChannelCode] = useState<string | undefined | null>(null);
  ...
  useEffect(() => {
    const timeout = setTimeout(() => {
      if (formik.values.idBrokerPsp && filterChannelCode !== null) {
        getChannelsByBrokerCode(formik.values.idBrokerPsp, filterChannelCode, true);
      }
    }, 300);
            ...
            <Grid container>
              ...
              <Grid item xs={6} sx={{ mt: 2, pr: 1 }}>
                <Autocomplete
                  ...
                  disabled={!formik.values.idBrokerPsp}
                  onChange={(_event, value) => handleChangeChannel(value)}
                  value={formik.values.idChannel}
                  fullWidth
                  filterOptions={(x) => x}
                  onInputChange={(_, value) => setFilterChannelCode(value)}
                  renderInput={(params) => (
                    <TextField
                      {...params}
