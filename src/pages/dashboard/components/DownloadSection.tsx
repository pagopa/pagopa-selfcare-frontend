...
const downloadIbansAsCSV = (brokerCode: string) => {
    exportIbanToCSV(brokerCode)
        .then((response) => {
            downloadBlobAsCSV(new Blob([new Uint8Array(response)], {type: 'text/csv'}), 'export-ibans.csv');
        })
        .catch((error) => {
            console.error(error);
        });
...
const downloadCreditorInstitutionsAsCSV = (brokerCode: string) => {
    exportCreditorInstitutionToCSV(brokerCode)
        .then((response) => {
            downloadBlobAsCSV(new Blob([new Uint8Array(response)], {type: 'text/csv'}), 'export-ci.csv');
        })
        .catch((error) => {
            console.error(error);
        });
