...
const StandInAndCupForm = ({ ibanList, error, loading }: Props) => {
  ...
  const submit = async () => {
    ...
    try {
      ...
      await updateNewIbans(standInInitialized, standInTouched, cupTouched, cupInitialized);
  ...
  async function updateNewIbans(
    standInInitialized: boolean,
    standInTouched: boolean,
    cupTouched: boolean,
    cupInitialized: boolean
  ) {
    if (newSelectedIban.cup.iban === newSelectedIban.standIn.iban) {
      ...
      if (
        standInTouched ||
        (originalSelectedIban.cup.iban === originalSelectedIban.standIn.iban && standInInitialized)
      ) {
      ...
      if (
        cupTouched ||
        (originalSelectedIban.cup.iban === originalSelectedIban.standIn.iban && cupInitialized)
      ) {
        await updateIban(newSelectedIban.cup.creditor_institution_code, {
          ...newSelectedIban.cup,
          labels: [cupLabel],
  ...
  async function resetOriginalIbans(
    standInInitialized: boolean,
    standInTouched: boolean,
    cupTouched: boolean,
    cupInitialized: boolean
            ...
            <Box display="flex" alignItems="center" mt={2}>
              ...
              {showManageButton ? (
                ...
                <FormControl sx={{ minWidth: '55%' }}>
                  <InputLabel size="small" id="ibanStandInLabel">
                    {t('ibanPage.selectIban')}
                  </InputLabel>
                  <Select
                    id="ibanStandIn"
                    name="ibanStandIn"
          ...
          <Grid item xs={12} md={6}>
            ...
            <Box display="flex" alignItems="center" mt={2}>
              ...
              {showManageButton ? (
                ...
                <FormControl sx={{ minWidth: '55%' }}>
                  <InputLabel size="small" id="ibanCupLabel">
                    {t('ibanPage.selectIban')}
                  </InputLabel>
                  <Select
                    id="ibanCup"
