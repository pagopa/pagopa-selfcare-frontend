...
const ServiceButton = ({ serviceInfo, onClick }: ({ serviceInfo: ServiceInfo; onClick: () => void })) => {
    ...
    if (serviceInfo.consent === ConsentEnum.OPT_IN) {
        return (<Button data-testid={`${serviceInfo.serviceId}-button-disabling`} variant="outlined" startIcon={<DoDisturbAltIcon />} color="error" onClick={onClick}>
            {t(`serviceConsent.${serviceInfo.serviceId}.disableButtonText`)}
        </Button>);
    } else {
        return (<Button data-testid={`${serviceInfo.serviceId}-button-enabling`} variant="contained" endIcon={<ArrowForwardIcon />} onClick={onClick}>
